<template>
  <section class="about" id="about">
    <div class="container">
      <p class="note">
        I have worked as mobile phones maintenance technician, I decided to
        shift my career to software development, my learning journey started by
        learning programming fundamentals using C++ to advanced topics like OOP,
        Inheritance, Polymorphism, Data structures & Algorithms.
      </p>
      <br />
      <p class="note">
        After that i decided to chose The Web development as my main field, and
        working as Fullstack & nodejs developer.
      </p>
      <p class="note">My Skills and knowledge in this feild is:</p>
      <ul>
        <li>Nodejs - Express(Good)</li>
        <li>Nest.js(Good)</li>
        <li>Vue - Vue Router - Pinia (Good)</li>
        <li>HTML & CSS(Good)</li>
        <li>Javascript(Good)</li>
        <li>TypeScript(Good)</li>
        <li>MongoDB(Good)</li>
        <li>PostgreSQL using TypeORM(Intermediate)</li>
        <li>Git - Github</li>
        <li>Swagger documentation interface</li>
      </ul>
      <br />
      <p>
        After all, I feel that I have learned a lot of things that took time to
        learn, but I have a passion to learn more features and skills that can
        help me to increase productivity and make work easier.
      </p>
    </div>
  </section>
</template>

<script>
export default {
  name: "AboutPage",
  data() {
    return {};
  },
};
</script>
<style></style>
